...
import { pullChipsAffiliate, searchAffiliate } from '../store/action';
...
const Page = () => {
  ...
  const handleOnSearch = async () => {
    if (data.userName) {
      const res = await d(searchAffiliate({ userName: data.userName.value }));
      if (!res.payload) {
  ...
  const handleSubmit = async ({amount, transactionType, userName}: any) => {
    ...
    bs.showBusy();
    try {
      unwrapResult(await d(pullChipsAffiliate({
