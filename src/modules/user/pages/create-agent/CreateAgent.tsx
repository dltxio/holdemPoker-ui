...
// Helpers
import { getCheckedModulesAff, getCurrentUserData } from '@/helpers/common';

// Actions
import { createAgent } from '../../store/action';
...
// Interfaces
import { ICreateAgentParam, ICreatedBy } from '../../interfaces/CreateAgent.interface';

import './CreateAgent.scss';

const Page = () => {
  const { affModules } = useAppSelector(store => store.global);
  console.log("affModules: ", affModules);
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      const res = unwrapResult(await dispatch(createAgent(params)));
  ...
  useEffect(() => {
    if (affModules && affModules.length) {
      setModules(getCheckedModulesAff(affModules, ROOT.filterModuleNewAff));
    }
  }, [affModules]);
